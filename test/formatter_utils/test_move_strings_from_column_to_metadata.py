...
class TestMoveStringsFromColumnToMetadata(unittest.TestCase):
    ...
    def test_move_strings_from_column_to_metadata(self, mock_stdout):
        data = pandas.DataFrame({'Floats+MSG': [1.1, "MSG", 3.3], 'Metadata': ["Metadata_str", None, None]})
        expected = pandas.DataFrame({'Floats+MSG': [1.1, None, 3.3], 'Metadata': ["Metadata_str\n\nMSG", None, None]},
                                    dtype=object)
        column_name = 'Floats+MSG'
        result = move_strings_from_column_to_metadata(data, column_name)
        pandas.testing.assert_frame_equal(result, expected)
        self.assertNotEqual(mock_stdout.getvalue(), f"\033[31mWARNING: Error adding Messages to Metadata \033[0m\n")
    ...
    def test_data_frame_empty(self, mock_stdout):
        ...
        self.assertEqual(mock_stdout.getvalue(), f"\033[31mWARNING: Error adding Messages to Metadata \033[0m\n")

    @patch('sys.stdout', new_callable=StringIO)
    def test_column_not_in_data_frame(self, mock_stdout):
        data = pandas.DataFrame({'Floats+MSG': [1.1, "MSG", 3.3], 'Metadata': ["Metadata_str", None, None]})
        expected = pandas.DataFrame({'Floats+MSG': [1.1, "MSG", 3.3], 'Metadata': ["Metadata_str", None, None]},
                                    dtype=object)
